...
def post_request(url, auth, header, payload, in_json):
    post_response_data = requests.post(url=url, auth=auth, headers=header, json=payload)
    if in_json is True:
...
def patch_request(url, auth, header, payload, in_json):
    patch_response_data = requests.patch(url=url, auth=auth, headers=header, json=payload)
    if in_json is True:
...
def delete_request(url, auth, header, payload, in_json):
    delete_response_data = requests.delete(url=url, auth=auth, headers=header, json=payload)
    if in_json is True:
